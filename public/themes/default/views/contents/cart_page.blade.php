    ...
    @if ($carts->count() > 0)
      @foreach ($carts as $cart)
        ...
        <div class="row shopping-cart-table-wrap mb-5 mt-3 {{ $expressId == $cart->id ? 'selected' : '' }}" id="cartId{{ $cart->id }}" data-cart="{{ $cart->id }}">
          <div class="col-md-8 shopping-cart-parent-section">
...
<!--                     <a href="{{ route('show.store', $cart->shop->slug) }}"> {{ $cart->shop->name }}</a>
 -->                    <a href="javascript:void(0)"> {{ $cart->shop->name }}</a>
                  @else
              ...
              <table class="table table shopping-cart-item-table" id="table{{ $cart->id }}">
                        <hr class="hr-header-separation">
              <thead class="border-bottom-null">
                <tr class="border-bottom-null">
                  ...
                  <th class="border-right-null">&nbsp;</th>
                </tr>
              </thead>

              <tbody class="border-top-null">
                @foreach ($cart->inventories as $item)
                  ...
                  <tr class="cart-item-tr border-top-null">
                    <td class="border-top-null" >
                      ...
                      <img src="{{ get_product_img_src($item, 'mini') }}" class="img-mini img-cart-item" alt="{{ $item->slug }}" title="{{ $item->slug }}" />
                    </td>

                    <td class="hidden-sm hidden-xs border-top-null">
                    ...
                    <td class="shopping-cart-item-price border-top-null">
                      <span>
                        {{ get_currency_prefix() }}
                    ...
                    <td class="border-top-null mt-10">
                    ...
                    <td class="border-top-null item-total-font ">
                      <span>
                        {{ get_currency_prefix() }}
                    ...
                    <td class="border-top-null">
                      <a href="javascript:void(0)" class="cart-item-remove border-right-null" data-cart="{{ $cart->id }}" data-item="{{ $item->id }}" data-toggle="tooltip" title="@lang('theme.remove_item')">&times;</a>
                    </td>
                  </tr> <!-- /.order-body -->
                @endforeach
              </tbody>
              ...
              <tfoot>
                <tr>
                  <td colspan="6" class="border-top-null">
        ...
        <div class="col-md-1 max-width-sub" ></div>
          <div class="col-md-3 space20">
            <div class="side-widget mt-xl" id="cart-summary{{ $cart->id }}">
              <h3 class="side-widget-title">
                <span class="cart-summary-heading">{{ trans('theme.cart_summary') }}</span>
              </h3>
            ...
            @if (allow_checkout())
              <button type="submit" form="formId{{ $cart->id }}" id="checkout-btn{{ $cart->id }}" class="btn btn-primary btn-sm flat pull-right btn-buy-from-this-seller"> 
             <!--   <button type="submit" form="javascript:void(0)" id="checkout-btn{{ $cart->id }}" class="btn btn-primary btn-sm flat pull-right btn-buy-from-this-seller"> -->
            ...
            @else
              <a href="#nav-login-dialog" data-toggle="modal" data-target="#loginModal" class="btn btn-primary btn-sm flat pull-right ">
               <!--   <a href="javascript:void(0)" data-toggle="modal" data-target="#loginModal" class="btn btn-primary btn-sm flat pull-right "> -->
                <i class="fa fa-shopping-cart"></i> {{ trans('theme.button.buy_from_this_seller') }}
              </a>
            @endif
          </div> <!-- /.col-md-3 -->
...
<!--           <a href="{{ url('/') }}" class="btn btn-black flat btn-continue-shopping">{{ trans('theme.button.continue_shopping') }}</a> -->
        <a href="javascript:void(0)" class="btn btn-black flat btn-continue-shopping">{{ trans('theme.button.continue_shopping') }}</a>
        </div>
        <div class="col-6 nopadding-left text-right">
          @if (is_incevio_package_loaded('checkout'))
           ...
           <!--  <a href="{{ route('oneCheckout') }}" id="allCheckoutBtn" class="btn btn-black btn-lg flat"> -->
             <a href="javascript:void(0)" id="allCheckoutBtn" class="btn btn-black btn-lg flat">
        ...
        <div class="col-12">
          <p class="lead text-center my-5">
           ...
           <!--  <a href="{{ url('/') }}" class="btn btn-primary flat"> -->
             <a href="javascript:void(0)" class="btn btn-primary flat">
