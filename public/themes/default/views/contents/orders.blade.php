    ...
    <table class="table" id="buyer-order-table">
      ...
      <tbody>
          <tr class="order-info-head">
            <td class="py-4 pl-5 mw_25rem-s">
              <h5 class="mb-2 fg_light_blue-s ">
            ...
            <td width="" class="store-info py-4 mw_19rem-s label-unpaid-cus">
              <h5 class="mb-2">
                <span>@lang('theme.store'):</span>
                @if ($order->shop->slug)
                  <a class="fg_light_blue-s" href="{{ route('show.store', $order->shop->slug) }}"> {{ $order->shop->name }}</a>
                @else
            ...
            <td width="" class="order-amount py-4 pr-xl-0 pr-5 text-center mw_16rem-s">
              ...
              <div class="text-center">
                <div class="btn-group" role="group">
                  <a class="btn btn-xs btn-default flat br_4px-s px-2  fg_black-s" href="{{ route('order.detail', $order) }}">@lang('theme.button.order_detail')</a>
                  @if ($order->dispute)
                    <a href="{{ route('dispute.open', $order) }}" class="btn btn-xs btn-default flat ml-2 br_4px-s px-2 fg_black-s" data-confirm="@lang('theme.confirm_action.open_a_dispute')">@lang('theme.dispute_detail')</a>
                  @else
                    <a href="{{ route('dispute.open', $order) }}" class="confirm btn btn-xs btn-default flat ml-2 br_4px-s px-2 fg_black-s" data-confirm="@lang('theme.confirm_action.open_a_dispute')">@lang('theme.button.open_dispute')</a>
                  @endif
                </div>
              </div>
            </td>
          ...
          @foreach ($order->inventories as $item)
            <tr class="order-body">
              ...
              @if ($loop->first)
                ...
                <td rowspan="{{ $loop->count }}" class="order-actions text-center px-5 mw_18rem-s" style="border-right:0px !important;">
                  <a href="{{ route('order.again', $order) }}" class="btn btn-default btn-sm btn-block br_4px-s">
                    {{-- <i class="fas fa-shopping-cart"></i> --}}
                     @lang('theme.order_again')
                  </a>
  
                  @unless($order->isCanceled())
                    <a href="{{ route('order.invoice', $order) }}" class="btn btn-default btn-sm btn-block br_4px-s">
                    ...
                    @if ($order->canBeCanceled())
                      ...
                      {!! Form::button('<i class="fas fa-times-circle-o"></i> ' . trans('theme.cancel_order'), ['type' => 'submit', 'class' => 'confirm btn btn-default btn-block br_4px-s', 'data-confirm' => trans('theme.confirm_action.cant_undo')]) !!}
                      {!! Form::close() !!}
  
                    @elseif($order->canRequestCancellation())
  
                      <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'cancel']) }}" class="modalAction btn btn-default btn-sm btn-block br_4px-s">
                        {{-- <i class="fas fa-times"></i> --}}
                         @lang('theme.cancel_items')</a>
  
                    @endif
  
                    @if ($order->canTrack())
                      <a href="{{ route('order.track', $order) }}" class="btn btn-black btn-sm btn-block br_4px-s">
                        {{-- <i class="fas fa-map-marker"></i> --}}
                         @lang('theme.button.track_order')
                      </a>
                    @endif
  
                    @if ($order->canEvaluate())
                      <a href="{{ route('order.feedback', $order) }}" class="btn btn-primary btn-sm btn-block br_4px-s">
                    ...
                    @if ($order->isFulfilled())
                      @if ($order->canRequestReturn())
                        <a href="{{ route('cancellation.form', ['order' => $order, 'action' => 'return']) }}" class="modalAction btn btn-default btn-sm btn-block br_4px-s">
                          {{-- <i class="fas fa-undo"></i> --}}
                           @lang('theme.return_items')</a>
                      @endif
  
                      @unless($order->goods_received)
                        {!! Form::model($order, ['method' => 'PUT', 'route' => ['goods.received', $order]]) !!}
                        {!! Form::button(trans('theme.button.confirm_goods_received'), ['type' => 'submit', 'class' => 'confirm btn btn-primary btn-block br_4px-s', 'data-confirm' => trans('theme.confirm_action.goods_received')]) !!}
                        {!! Form::close() !!}
                      @endunless
                    @endif
                  ...
                  <a href="{{ route('order.detail', $order) . '#message-section' }}" class="btn btn-link btn-block mt-0 fs_12px-s fg_light_blue-s">
