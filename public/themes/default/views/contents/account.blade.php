<div role="tabpanel" class='account__myaccount account__myaccount_custom'>
  <ul class="nav nav-tabs" role="tablist">
    ...
    <li role="presentation" class="account-li-cus">
  ...
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane fade active in" id="account-info-tab">
      <div class="row">
        <div class="col-md-8">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8 text-right">
              {!! Form::submit(trans('theme.button.update'), ['class' => 'btn btn-primary btn-account-custom']) !!}
            </div>
          </div>
          {!! Form::close() !!}
        </div><!-- /.col-md-8 -->
        ...
        <div class="col-md-4">
          <div class="user-avatar-section text-center account-avatar-custom">
            <div class="form-group">
              @if ($account->image)
              ...
              <img src="{{ get_storage_file_url(optional($account->image)->path, 'medium') }}" class="thumbnail mb-2" alt="{{ trans('theme.avatar') }}" />
              {!! Form::label('avatar', trans('theme.avatar')) !!}
            </div>

            {!! Form::open(['route' => 'my.avatar.save', 'files' => true, 'class' => '', 'data-toggle' => 'validator']) !!}
            <div class="form-group ml-5 mb-4 mt-5">
    ...
    <div role="tabpanel" class="tab-pane fade" id="address-tab">
      <div class="row">
        <div class="col-md-10 col-sm-offset-1 space30 address-tab-custom">
          @forelse($account->addresses as $address)
            {!! $address->toHtml() !!}
            <div class="btn-group pull-right space20" role="group" aria-label="..." style="margin-top: -100px;">
              <a href="{{ route('my.address.delete', $address->id) }}" class="confirm btn btn-default btn-xs ml-2 pull-right" data-confirm="@lang('theme.confirm_action.delete')">
                <i class="fas fa-trash-o"></i> @lang('theme.button.delete')
              </a>

              <a href="{{ route('my.address.edit', $address) }}" class="modalAction btn btn-default btn-xs pull-right">
